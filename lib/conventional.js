        ...
        if (!fs.existsSync(configFile)) {
            ...
            fs.readFile(`${workingDir}/lib/fixtures/${fileName}`, read = (err, data) => {
                ...
                prompt.get([
                    ...
                    //if prompt canceled do nothing and exit
                    if(err || !result) {
                        return false;
                    }
            ...
            fs.readFile(configFile, read = (err, data) => {
                ...
                if(!validatePackageJson(json)) {
                    ...
                    prompt.get([
                        {name: 'y', message: `It appears that your package.json is not prepared for the conventional building, would you like to update it? (y/n)`}
                    ], function (err, result) {
